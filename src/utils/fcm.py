...
class Dfcm():
    ...
    # FIT BEGIN ==============================================
    def __fit_with_centroid(self, init_v: np.ndarray = None, seed: int = 0, device: str = 'CUDA'):
    ...
    def __fit_with_membership(self, init_u: np.ndarray = None, seed: int = 0, device: str = 'CUDA'):
    ...
    def fit(self, data: np.ndarray, init_u: np.ndarray = None, init_v: np.ndarray = None, seed: int = 0, with_u: bool = True, device: str = 'CUDA') -> tuple:
    ...
    def map_clusters_to_labels(self, cluster_labels, true_labels):
        """
        Map cụm sau khi phân cụm với nhãn thực tế sử dụng Hungarian Algorithm
        
        Args:
            cluster_labels: Nhãn cụm từ FCM [0, 1, 0, ...]
            true_labels: Nhãn thực tế [0, 1, 0, ...]
        
        Returns:
            mapped_labels: Nhãn cụm đã map [0, 1, 0, ...]
            mapping: Dict mapping {cụm_0: nhãn_thực, cụm_1: nhãn_thực}
        """
        # Tạo confusion matrix
        cm = confusion_matrix(true_labels, cluster_labels)
        
        # Sử dụng Hungarian Algorithm để tìm mapping tối ưu
        row_ind, col_ind = linear_sum_assignment(-cm)  # Dấu - để maximize
